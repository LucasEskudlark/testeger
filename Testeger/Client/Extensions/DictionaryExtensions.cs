...
public static class DictionaryExtensions
{
    ...
    public static string GetTestCaseTimeByStatus(this Dictionary<TestCaseStatus, TimeSpan> dictionary, TestCaseStatus status)
    {
        if (dictionary.TryGetValue(status, out TimeSpan timeSpan))
        {
            return timeSpan.ToCustomFormat();
    ...
    public static double GetTestCaseTimeByStatusForChart(this Dictionary<TestCaseStatus, TimeSpan> dictionary, TestCaseStatus status)
    {
        if (dictionary.TryGetValue(status, out TimeSpan timeSpan))
        {
            return Math.Round(timeSpan.TotalHours, 2);
