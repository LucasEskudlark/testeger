...
public class ThemeService
{
    ...
    public void Initialize(NavigationManager navigationManager)
    {
        ...
        {
            CurrentTheme = DefaultTheme; // Set default theme if no theme is set in the query parameters
        }

        WCAG = bool.Parse(query.Get(WCAGQueryParameter) ?? "false");
    }

    public void SetDefaultThemeIfNotSet(NavigationManager navigationManager, bool wcag = false)
    {
        var uri = new Uri(navigationManager.ToAbsoluteUri(navigationManager.Uri).ToString());
        var query = HttpUtility.ParseQueryString(uri.Query);
        var currentTheme = query.Get(QueryParameter);

        if (currentTheme != DefaultTheme)
        {
            Change(navigationManager, DefaultTheme, wcag);
        }
    }
